...
import { Plus, Search, MoreHorizontal, Edit, Trash2, Download, SortAsc, SortDesc } from "lucide-react"
  ...
  useEffect(() => {
    ...
    if (searchTerm) {
      filtered = filtered.filter(
        (t) =>
          t.description.toLowerCase().includes(searchTerm.toLowerCase()) || getCategoryName(t.category).toLowerCase().includes(searchTerm.toLowerCase())
      )
    }
